                ...
                destinationWB.xlsx.readFile(`${process.env.OUTPUT_FILE}`).then(async() => {
                    ...
                    destinationSheet.eachRow({ includeEmpty: false, firstRow: 2}, (row, rowNumber) => {
                        if (rowNumber > 1) {
                            ...
                            row.getCell(18).value = { formula: `IF(VLOOKUP(E${rowNumber},Sku_Consolidated!A2:S${consolidatedSheet.lastRow.number},19,FALSE)="SHOWCASE", VLOOKUP(D${rowNumber},Store_Showcase!C2:N${showcaseSheet.lastRow.number},{7}, FALSE), IF(VLOOKUP(E${rowNumber},Sku_Consolidated!A2:S${consolidatedSheet.lastRow.number},19,FALSE)="VAM",VLOOKUP(D${rowNumber},Store_VAM!C2:N${vamSheet.lastRow.number},{7}, FALSE),VLOOKUP(D${rowNumber},Store_SRP!C2:N${srpSheet.lastRow.number},{7}, FALSE)))`}; // PLACEMENT
