                ...
                destinationWB.xlsx.readFile(`${process.env.OUTPUT_FILE}`).then(async() => {
                    ...
                    destinationSheet.eachRow({ includeEmpty: false, firstRow: 2}, (row, rowNumber) => {
                        if (rowNumber > 1) {
                            ...
                            //  // UOM
                            row.getCell(10).value = { formula: `VLOOKUP(G${rowNumber},Sku_Consolidated!A2:C${consolidatedSheet.lastRow.number}, 3, FALSE)`};
                            // PACK
                            row.getCell(11).value = { formula: `IF(J${rowNumber}="PACK", I${rowNumber}, 0)`};
                            row.getCell(11).numFmt = `###0.00000`;
                            row.getCell(11).alignment = { horizontal: 'right' };
                            // KG
                            row.getCell(12).value = { formula: `IF(J${rowNumber}="KG", I${rowNumber}, I${rowNumber} * VLOOOKUP(G${rowNumber},Sku_Consolidated!A2:U${consolidatedSheet.lastRow.number},21,FALSE))`};
                            row.getCell(12).numFmt = `###0.00000`;
                            row.getCell(12).alignment = { horizontal: 'right' };
                            // PCS
                            row.getCell(13).numFmt = `###0.00000`; 
                            ...
                            row.getCell(16).numFmt = `###0.00000`;
                            row.getCell(16).alignment = { horizontal: 'right' };
                            // SKU CATEGORY
                            row.getCell(17).value = { formula: `VLOOKUP(G${rowNumber},Sku_Consolidated!A2:G${consolidatedSheet.lastRow.number},7, FALSE)`};
                            // AREA
                            ...
                            // SKU IDENTIFIER 1
                            row.getCell(20).value = { formula: `VLOOKUP(G${rowNumber},Sku_Consolidated!A2:R${consolidatedSheet.lastRow.number},18,FALSE)`};
                            // BANNER
                            row.getCell(22).value = { formula: `IF(VLOOKUP(G${rowNumber},Sku_Consolidated!A2:S${consolidatedSheet.lastRow.number},19,FALSE)="SHOWCASE", VLOOKUP(E${rowNumber},Store_Showcase!B2:L${showcaseSheet.lastRow.number},11, FALSE), IF(VLOOKUP(G${rowNumber},Sku_Consolidated!A2:S${consolidatedSheet.lastRow.number},19,FALSE)="VAM",VLOOKUP(E${rowNumber},Store_VAM!B2:L${vamSheet.lastRow.number},11, FALSE),VLOOKUP(E${rowNumber},Store_SRP!B2:L${srpSheet.lastRow.number},11, FALSE)))`};
                            // SKU PER BRAND
                            row.getCell(23).value = { formula: `VLOOKUP(G${rowNumber},Sku_Consolidated!A2:W${consolidatedSheet.lastRow.number},11,FALSE)`};
                            // GENERALIZED SKU
                            row.getCell(24).value = { formula: `VLOOKUP(G${rowNumber},Sku_Consolidated!A2:H${consolidatedSheet.lastRow.number},8,FALSE)`};
                            // MOTHER SKU
                            row.getCell(25).value = { formula: `VLOOKUP(G${rowNumber},Sku_Consolidated!A2:I${consolidatedSheet.lastRow.number},9,FALSE)`};
                            // SKU DEPT
                            row.getCell(27).value = { formula: `VLOOKUP(G${rowNumber},Sku_Consolidated!A2:E${consolidatedSheet.lastRow.number},5,FALSE)`};
                            // PLACEMENT
                            ...
                            // PLACEMENT REMARKS
                            row.getCell(29).value = { formula: `IF(IFERROR(AB${rowNumber},TRUE)=TRUE, "-","OK")`};
                            // NET TAX
