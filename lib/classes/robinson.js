            ...
            return await sourceWB.xlsx.readFile(sourceFile).then(() => {
                ...
                destinationWB.xlsx.readFile(`${process.env.OUTPUT_FILE}`).then(async() => {
                    ...
                    destinationSheet.eachRow({ includeEmpty: false, firstRow: 2}, (row, rowNumber) => {
                        if (rowNumber > 1) {
                            row.getCell(12).value = { formula: `IF(J${rowNumber}="KLS", I${rowNumber}, IF(J${rowNumber}="PCK", I${rowNumber}*VLOOKUP(G${rowNumber},Sku_Consolidated!A2:U${consolidatedSheet.lastRow.number},21, FALSE),I${rowNumber}*VLOOKUP(G${rowNumber},Sku_Consolidated!A2:U${consolidatedSheet.lastRow.number},21, FALSE)))`} // KLS
                            row.getCell(12).numFmt = `###0.00000`; // KLS FORMAT
                            // row.getCell(15).value = { formula: `N${rowNumber} * VLOOKUP(VLOOKUP(G${rowNumber},Sku_Consolidated!A2:Q${consolidatedSheet.lastRow.number},17,FALSE),Sku_CommRate!D2:E${commrateSheet.lastRow.number},2,FALSE)`} // COMM RATE
                            row.getCell(15).value = { formula: `N${rowNumber} * VLOOKUP(E${rowNumber}&VLOOKUP(G${rowNumber},Sku_Consolidated!A2:Q${consolidatedSheet.lastRow.number},17,FALSE),Sku_CommRate!A2:E${commrateSheet.lastRow.number}, 5, FALSE)`} // COMM RATE
                            ...
                            // row.getCell(22).value = { formula: `IF(IFERROR(VLOOKUP(E${rowNumber},Store_Showcase!B2:L${showcaseSheet.lastRow.number},11, FALSE), TRUE)=TRUE, IF(IFERROR(VLOOKUP(E${rowNumber},Store_SRP!B2:K${srpSheet.lastRow.number},10, FALSE), TRUE)=TRUE,VLOOKUP(E${rowNumber},Store_VAM!B2:K${vamSheet.lastRow.number},10, FALSE),VLOOKUP(E${rowNumber},Store_SRP!B2:K${srpSheet.lastRow.number},10, FALSE)), VLOOKUP(E${rowNumber},Store_Showcase!B2:L${showcaseSheet.lastRow.number},11, FALSE))`} // BANNER
