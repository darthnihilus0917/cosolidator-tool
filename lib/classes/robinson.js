...
class Robinson {
                    ...
                    if (duplicated) {
                        this.checkFileExists(async(err, exists) => {
                            if (err) {
                                ...
                                await tempoWB.xlsx.readFile(`${process.env.OUTPUT_FILE_ROBINSON}`).then(() => {
                                    const tempoSheet = tempoWB.getWorksheet('Robinson');        
                                    const tempoWBUpdate = XLSX.readFile(`${process.env.OUTPUT_FILE_ROBINSON}`);
                                    ...
                                    tempoSheet.eachRow({includeEmpty: false }, (row, rowNumber) => {
                                        if (rowNumber !== 1) {
                                            ...
                                            tempoWBUpdateSheet[`O${rowNumber}`] = { t: 'n', f: `N${rowNumber} * VLOOKUP(VLOOKUP(G${rowNumber},Sku_Consolidated!A2:Q${consolidatedSheet.lastRow.number},17,FALSE),Sku_CommRate!D${rowNumber}:E${commrateSheet.lastRow.number},2,FALSE)`}
                                            tempoWBUpdateSheet[`P${rowNumber}`] = { z: '###0.00000', t: 'n', f: `N${rowNumber} - O${rowNumber}`}
                                        }
                                    })
                                    XLSX.writeFile(tempoWBUpdate, `${process.env.OUTPUT_FILE_ROBINSON}`)
                                });                                   
                            }
                        });
                    }
    ...
    duplicateSheetData(targetFile) {
        ...
        XLSX.writeFile(newWB, `${process.env.OUTPUT_FILE_ROBINSON}`);
    }
