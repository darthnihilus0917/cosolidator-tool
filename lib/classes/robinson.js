...
class Robinson {
                ...
                destinationWB.xlsx.readFile(`${process.env.OUTPUT_FILE}`).then(async() => {
                    ...
                    destinationSheet.eachRow({ includeEmpty: false, firstRow: 2}, (row, rowNumber) => {
                        if (rowNumber > 1) {
                            ...
                            // NET TAX
                            row.getCell(30).numFmt = `###0.00000`;
                            row.getCell(30).alignment = { horizontal: 'right' };
                            // TAX
                            row.getCell(31).numFmt = `###0.00000`;
    ...
    removeUnrelatedSheets() {
        ...
        workbook.xlsx.readFile(`${process.env.OUTPUT_FILE_ROBINSON}`).then(() => {
    ...
    checkFileExists(callback) {
        ...
        function check() {
            fs.access(`${process.env.OUTPUT_FILE_ROBINSON}`, fs.constants.F_OK, (err) => {
                if (!err) {
                    // File exists
                    callback(null, true);
                } else {
