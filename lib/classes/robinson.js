...
class Robinson {
    ...
    async consolidate() {
        try {
            const sourceFile = `${process.env.OUTPUT_FILE_ROBINSON}`;
            const sourceWB = new ExcelJS.Workbook();

            const fileManager = new DataFiles();
            fileManager.source = `${process.env.OUTPUT_DIR}`;
            ...
            if (fileResult.length > 0) {
                return await sourceWB.xlsx.readFile(sourceFile).then(() => {
                    ...
                    //     const rowData = [11].map(col => row.getCell(col).value).filter(r => r !== undefined);
