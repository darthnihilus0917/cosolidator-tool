...
class Robinson {
    ...
    finalizeOutputData() {
        const destinationWBUpdate = XLSX.readFile(`./output/tempo_sheet.xlsx`);
        const robinsonSheet = destinationWBUpdate.Sheets['Robinson'];

        const csvData = XLSX.utils.sheet_to_csv(robinsonSheet);

        fs.writeFileSync('./robinson_output_data.csv', csvData, 'utf8');
        // const robinsonData = XLSX.utils.sheet_to_json(robinsonSheet, { raw: true });

        // const newWB = XLSX.utils.book_new();
        // const newRobinsonSheet = XLSX.utils.json_to_sheet(robinsonData);
        // XLSX.utils.book_append_sheet(newWB, newRobinsonSheet, 'Robinson');
        // XLSX.writeFile(newWB, './robinson_output_data.xlsx');
    }
