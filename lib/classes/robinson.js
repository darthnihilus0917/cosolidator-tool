...
class Robinson {
                    ...
                    if (duplicated) {
                        this.checkFileExists(async(err, exists) => {
                            if (err) {
                                ...
                                await tempoWB.xlsx.readFile(`./output/robinson_output_data.xlsx`).then(() => {
                                    const tempoSheet = tempoWB.getWorksheet('Robinson');        
                                    const tempoWBUpdate = XLSX.readFile('./output/robinson_output_data.xlsx');
                                    ...
                                    tempoSheet.eachRow({includeEmpty: false }, (row, rowNumber) => {
                                        if (rowNumber !== 1) {
                                            ...
                                            tempoWBUpdateSheet[`O${rowNumber}`] = { z: '###0.00000', t: 'n', f: `N${rowNumber} * VLOOKUP(VLOOKUP(G${rowNumber},Sku_Consolidated!A2:Q${consolidatedSheet.lastRow.number},17,FALSE),Sku_CommRate!D${rowNumber}:E${commrateSheet.lastRow.number},2,FALSE)`}
                                            tempoWBUpdateSheet[`P${rowNumber}`] = { z: '###0.00000', t: 'n', f: `N${rowNumber} - O${rowNumber}`}
                                        }
                                    })
                                    XLSX.writeFile(tempoWBUpdate, './output/robinson_output_data.xlsx')
                                });                                   
                            }
                        });
                    }
    ...
    duplicateSheetData(targetFile) {
        ...
        XLSX.writeFile(newWB, './output/robinson_output_data.xlsx');
    }
    ...
    checkFileExists(callback) {
        ...
        function check() {
            fs.access(`./output/robinson_output_data.xlsx`, fs.constants.F_OK, (err) => {
                if (!err) {
                    // File exists
                    callback(null, true);
                } else {
