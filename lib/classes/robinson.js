                ...
                destinationWB.xlsx.readFile(`${process.env.OUTPUT_FILE}`).then(async() => {
                    ...
                    destinationSheet.eachRow({ includeEmpty: false, firstRow: 2}, (row, rowNumber) => {
                        if (rowNumber > 1) {
                            ...
                            row.getCell(19).value = { formula: `IF(VLOOKUP(G${rowNumber},Sku_Consolidated!A2:S${consolidatedSheet.lastRow.number},19,FALSE)="SHOWCASE", VLOOKUP(E${rowNumber},Store_Showcase!B2:N${showcaseSheet.lastRow.number},{9}, FALSE), IF(VLOOKUP(G${rowNumber},Sku_Consolidated!A2:S${consolidatedSheet.lastRow.number},19,FALSE)="VAM",VLOOKUP(E${rowNumber},Store_VAM!B2:N${vamSheet.lastRow.number},{9}, FALSE),VLOOKUP(E${rowNumber},Store_SRP!B2:N${srpSheet.lastRow.number},{9}, FALSE)))`}; // KAM
