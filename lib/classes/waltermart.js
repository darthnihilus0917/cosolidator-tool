                ...
                destinationWB.xlsx.readFile(`${process.env.OUTPUT_FILE}`).then(async() => {
                    ...
                    destinationSheet.eachRow({ includeEmpty: false, firstRow: 2}, (row, rowNumber) => {
                        if (rowNumber > 1) {
                            ...
                            row.getCell(23).value = { formula: `IF(VLOOKUP(I${rowNumber},Sku_Consolidated!A2:S${consolidatedSheet.lastRow.number},19,FALSE)="SHOWCASE", VLOOKUP(G${rowNumber},Store_Showcase!B2:N${showcaseSheet.lastRow.number},8, FALSE), IF(VLOOKUP(I${rowNumber},Sku_Consolidated!A2:S${consolidatedSheet.lastRow.number},19,FALSE)="VAM",VLOOKUP(G${rowNumber},Store_VAM!B2:N${vamSheet.lastRow.number},8, FALSE),VLOOKUP(G${rowNumber},Store_SRP!B2:N${srpSheet.lastRow.number},8, FALSE)))`}; // KAM
