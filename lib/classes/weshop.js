                ...
                destinationWB.xlsx.readFile(`${process.env.OUTPUT_FILE}`).then(async() => {
                    ...
                    destinationSheet.eachRow({ includeEmpty: false, firstRow: 2}, (row, rowNumber) => {
                        if (rowNumber > 1) {                            
                            ...
                            row.getCell(13).value = { formula: `IF(L${rowNumber}="PCK", K${rowNumber}, IF(L${rowNumber}="PCS", K${rowNumber}, 0))`};

                            row.getCell(14).numFmt = `###0.00000`; // KG
                            row.getCell(14).alignment = { horizontal: 'right' };
                            const packCheck = `IF(L${rowNumber}="PCK",K${rowNumber}*VLOOKUP(F${rowNumber},Sku_Consolidated!B2:U${consolidatedSheet.lastRow.number}, {20, 2}, FALSE),0)`;
                            const pcsCheck = `IF(L${rowNumber}="PCS",K${rowNumber}*VLOOKUP(F${rowNumber},Sku_Consolidated!B2:U${consolidatedSheet.lastRow.number}, {20, 2}, FALSE),${packCheck})`;                            
                            row.getCell(14).value = { formula: `IF(L${rowNumber}="Gs",K${rowNumber}*VLOOKUP(F${rowNumber},Sku_Consolidated!B2:U${consolidatedSheet.lastRow.number}, {20, 2}, FALSE),${pcsCheck})`};
                            ...
                            const srpKamCheck = `VLOOKUP(W${rowNumber},Store_SRP!C2:I${srpSheet.lastRow.number},{7}, FALSE)`;
                            const vamKamCheck = `IF(VLOOKUP(F${rowNumber},Sku_Consolidated!B2:S${consolidatedSheet.lastRow.number},{18},FALSE)="VAM",VLOOKUP(W${rowNumber},Store_VAM!C2:I${vamSheet.lastRow.number},7, FALSE),${srpKamCheck})`;
                            const showcaseKamCheck = `IF(VLOOKUP(F${rowNumber},Sku_Consolidated!B2:S${consolidatedSheet.lastRow.number},{18},FALSE)="SHOWCASE",VLOOKUP(W${rowNumber},Store_Showcase!C2:I${showcaseSheet.lastRow.number},7, FALSE),${vamKamCheck})`;
                            const chickenKamCheck = `IF(VLOOKUP(F${rowNumber},Sku_Consolidated!B2:S${consolidatedSheet.lastRow.number},{18},FALSE)="CHICKEN",VLOOKUP(W${rowNumber},Store_Showcase!C2:I${showcaseSheet.lastRow.number},7, FALSE),${showcaseKamCheck})`;                            
                            row.getCell(22).value = { formula: `IF(VLOOKUP(F${rowNumber},Sku_Consolidated!B2:S${consolidatedSheet.lastRow.number},{18},FALSE)="EGG",VLOOKUP(W${rowNumber},Store_Showcase!C2:I${showcaseSheet.lastRow.number},7, FALSE),${chickenKamCheck})`};
