            ...
            return await sourceWB.xlsx.readFile(sourceFile).then(() => {
                ...
                destinationWB.xlsx.readFile(`${process.env.OUTPUT_FILE}`).then(async() => {
                    ...
                    destinationSheet.eachRow({ includeEmpty: false, firstRow: 2}, (row, rowNumber) => {
                        if (rowNumber > 1) {
                            row.getCell(20).value = { formula: `VLOOKUP(F${rowNumber},Sku_Consolidated!B2:L${consolidatedSheet.lastRow.number}, {6}, FALSE)`} // SALES CATEGORY
                            ...
                            row.getCell(30).value = { formula: `VLOOKUP(F${rowNumber},Sku_Consolidated!B2:L${consolidatedSheet.lastRow.number}, {3}, FALSE)`} // SKU DEPT.
                            row.getCell(32).value = "OK" // SKU PER BRAND
                        }
                    });
