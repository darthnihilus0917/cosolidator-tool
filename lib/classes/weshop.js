                ...
                destinationWB.xlsx.readFile(`${process.env.OUTPUT_FILE}`).then(async() => {
                    ...
                    destinationSheet.eachRow({ includeEmpty: false, firstRow: 2}, (row, rowNumber) => {
                        if (rowNumber > 1) {                            
                            ...
                            // SKU CATEGORY
                            row.getCell(20).value = { formula: `VLOOKUP(F${rowNumber},Sku_Consolidated!B2:L${consolidatedSheet.lastRow.number}, {6}, FALSE)`};
                            // KAM
                            row.getCell(22).value = { formula: `VLOOKUP(W${rowNumber},Store_Showcase!C2:I${showcaseSheet.lastRow.number},7, FALSE)`};
                            // SKU PER BRAND
                            row.getCell(26).value = { formula: `VLOOKUP(F${rowNumber},Sku_Consolidated!B2:L${consolidatedSheet.lastRow.number}, {10, 2}, FALSE)`};
                            // GENERALIZED SKU
                            row.getCell(27).value = { formula: `VLOOKUP(F${rowNumber},Sku_Consolidated!B2:L${consolidatedSheet.lastRow.number}, {7}, FALSE)`};
                            // MOTHER SKU
                            row.getCell(28).value = { formula: `VLOOKUP(F${rowNumber},Sku_Consolidated!B2:L${consolidatedSheet.lastRow.number}, {8}, FALSE)`};
                            // SKU DEPT.
                            row.getCell(30).value = { formula: `VLOOKUP(F${rowNumber},Sku_Consolidated!B2:L${consolidatedSheet.lastRow.number}, {3}, FALSE)`};
                            // PLACEMENT
                            ...
                            // PLACEMENT REMARKS
                            row.getCell(32).value = { formula: `IF(IFERROR(AE${rowNumber},TRUE)=TRUE, "-","OK")`};
                        }
                    });
